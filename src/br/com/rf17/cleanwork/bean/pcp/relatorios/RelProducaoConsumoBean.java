...
@ManagedBean
@ViewScoped
public class RelProducaoConsumoBean implements Serializable {
	...
	public void imprimir() {
		try{       
        	...
        	if(produto != null){
        		filtroSql.append(" AND c.id_produto = " + produto.getId_produto());
        	}
            ...
            session.setAttribute("relatorio", "RelProducao_consumido.jasper");
	...
	public void pesquisaGenericaSelect() {
		try {
			if (tipoPesquisa.equals("produtoinsumo")) {
				...
				RequestContext.getCurrentInstance().update("form_data_relProducaoConsumo:dados_produto");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
